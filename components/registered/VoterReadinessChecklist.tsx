...
const STORAGE_KEY = "votesaathi_readiness";
...
export default function VoterReadinessChecklist() {
  ...
  return (
    ...
    >
      <div className="flex items-start justify-between mb-5">
        ...
        <div className="relative w-[72px] h-[72px] shrink-0">
          ...
          <div className="absolute inset-0 flex items-center justify-center">
            <span className={`text-lg font-bold font-mono-data ${score === 5 ? "text-[#138808]" : "text-[#FF6B00]"}`}>
              {score}/5
            </span>
          </div>
        </div>
      ...
      {score === 0 ? (
        <EmptyState
          variant="checklist-empty"
          title={t("readiness.emptyTitle")}
          description={t("readiness.emptyDesc")}
        />
      ) : null}
          ...
          >
            ...
            <span
              className={`text-sm transition-colors ${
                checked[i] ? "text-slate-300 line-through" : "text-slate-200"
              }`}
            >
              {t(key)}
            </span>
        ...
        {score === 5 && (
          ...
          >
            <div className="rounded-xl p-4 border border-[#138808]/30" style={{ background: "linear-gradient(135deg, rgba(19,136,8,0.15), rgba(19,136,8,0.05))" }}>
              <p className="text-sm font-semibold text-[#138808] mb-1">
                {t("readiness.ready")}
              </p>
              <p className="text-xs text-slate-300 mb-3">
                {t("readiness.shareText").replace("{date}", electionState.votingDeadline)}
              </p>
              <button
                onClick={handleCopy}
                className="flex items-center gap-2 text-xs font-medium px-4 py-2 rounded-lg transition-all"
                style={{
                  background: copied ? "rgba(19,136,8,0.3)" : "rgba(19,136,8,0.2)",
                  border: "1px solid rgba(19,136,8,0.4)",
                  color: "#22c55e",
                }}
                id="readiness-copy-btn"
              >
                {copied ? (
                  <>
